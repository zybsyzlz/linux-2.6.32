...
/*parse_options:分析data参数，并设置相关变量*/
...
/*update_special:更新special子目录的访问模式*/
...
/*remount:改变usbfs文件系统挂载点中目录项的访问模式*/
...
/*usbfs_get_inode:在sb超级块中获得一个空闲的inode节点*/
...
static int usbfs_mkdir (struct inode *dir, struct dentry *dentry, int mode)
{
	...
	if (!res)
		inc_nlink(dir); /*增加目录的硬链接数*/
	return res;
}

/*usbfs_create:在dir目录项下创建dentry文件*/
...
/*
 * fs_create_by_name :在parent目录下创建name文件（目录）
 * @name: 文件(目录)名称	
 * @mode: 访问模式
 * @parent:父目录
 * @dentry:返回创建的文件(目录)的目录项节点	
 */
			      ...
			      struct dentry *parent, struct dentry **dentry)
{
	...
	/*如果parent == NULL,则在usbfs文件系统的根目录下创建文件和目录*/
	if (!parent ) {
		if (usbfs_mount && usbfs_mount->mnt_sb) {
...
/*fs_create_file:在parent目录项下创建文件(目录)*/
...
/*create_special_files:创建usbfs的文件系统的挂载点*/
...
/*remove_special_files:删除usbfs文件系统的挂载点*/
static void remove_special_files (void)
{
	/*删除usbfs文件系统根目录下的devices文件节点*/
...
/**
 * usbfs_update_special:更新usbfs文件系统中"devices"文件节点的访问时间
 */
...
/*usbfs_add_bus:在usbfs文件系统的挂载点的根目录下创建bus编号的目录*/
...
/*usbfs_remove_bus:删除bus->usbfs_dentry目录项*/
...
/*usbfs_add_device:在usbfs文件系统中创建以设备号为名字的文件*/
static void usbfs_add_device(struct usb_device *dev)
{
	 ...
	 * 文件的长度为设备描述符+设备的configurature的所有
	 * 长度*/
...
/*usbfs_remove_device:删除dev->usbfs_dentry目录项*/
...
/*usbfs_notify:usbfs文件系统通知连回调函数接口*/
