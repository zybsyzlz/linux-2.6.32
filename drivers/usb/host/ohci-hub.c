...
__acquires(ohci->lock)
{
	...
	case OHCI_USB_RESUME:
		...
		/*when hc is reset ,it is suspended*/
...
__acquires(ohci->lock)
{
	 ...
	 * irqs disabled if and only if autostopped is true,the host
	 * may turn schedules back on
	 * 主机设置为oper模式，继续传输. 
	 */
...
/**
 * ohci_root_hub_state_changes:根据hub端口状态判断hub是否需要轮寻 
 * @ ohci: ohci主机
 * @ changed: 
 * @ any_connected:hub端口是否有设备的链接标记
 * @ rhsc_status:hub或者hub端口状态改变
 */
...
/**
 * ohci_root_hub_state_changes:判断hub的状态是否需要轮寻 
 * @ changed:
 * @ any_connected:
 * @ rhsc_status:hub或者hub端口状态发生变化
 */
static int ohci_root_hub_state_changes(struct ohci_hcd *ohci, int changed,
		int any_connected, int rhsc_status)
{
...
/**
 * ohci_hub_status_data: 返回hub的状态信息，保存在buf中
 */
...
/**
 * ohci_hub_descriptor:设置hub的设备描述符
 * @ ohci: ohci主机
 * @ desc: hub设备描述符结构
 */
...
/**
 * ohci_hub_control:hub传输请求控制接口
 * @ typeRwq:请求类型
 */
static int ohci_hub_control (struct usb_hcd	*hcd, u16 typeReq, u16 wValue,
