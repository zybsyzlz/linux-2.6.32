...
#define ED_H		(0x01)			/* halted */
	...
	struct list_head	td_list;	/* "shadow list" of our TDs td的传输队列链表*/
...
#define ED_OPER		0x02		/* IS linked to hc */
	...
	u8			type;		/* PIPE_{BULK,...} PIPE 传输类型 */

	/* periodic scheduling params (for intr and iso) */
	u8			branch;
	u16			interval;
	u16			load;     /*总线传输时间(微秒为单位)bus time in ns*/
	u16			last_iso;	/* iso only */

	/* HC may see EDs on rm_list until next frame (frame_no == tick) */
	u16			tick; /*tick = current frame number + 1*/
...
#define TD_EC       (0x3 << 26)			/* error count */
#define TD_T        (0x3 << 24)			/* data toggle state */
#define TD_T_DATA0  0x02000000				/* DATA0 */
#define TD_T_DATA1  0x03000000				/* DATA1 */
#define TD_T_TOGGLE 0x00000000				/* uses ED_C */
#define TD_DP       (0x3 << 19) 			/* direction/pid */
#define TD_DP_SETUP (0x0 << 19) 			/* SETUP pid */
#define TD_DP_IN    (0x2 << 19) 			/* IN pid */
#define TD_DP_OUT   (0x1 << 19) 			/* OUT pid */
#define TD_DP_RSVD  (0x3 << 19) 			/* Reserved pid */
#define TD_R        (0x1 << 18)			/* round: short packets OK? */
...
#define MAXPSW	2
	...
	__u8		index;
	struct ed	*ed;
	struct td	*td_hash;	/* dma-->td hashtable for ohci's td hash list*/
...
typedef struct urb_priv {
	struct ed		*ed;
	u16			length;		// # tds in this request td的总个数
	u16			td_cnt;		// tds already serviced  td已经完成传输的个数
	struct list_head	pending;
	struct td		*td [0];	// all TDs in this request length个td结构数组
...
struct ohci_hcd {
	...
	struct ed		*ed_rm_list;		/*存放将要删除的ed*/
	...
	unsigned long		next_statechange;	/* suspend/resume 的时间*/
...
static inline void ohci_halt (struct ohci_hcd *ohci)
{
