...
static unsigned int nr_swapfiles; /*保存最后一个交换区在交换区数组中的索引*/
long nr_swap_pages;
...
/**
 * scan_swap_map:获得交换区si的一个空闲页槽的索引
 */
...
scan:
	spin_unlock(&swap_lock);
	/*遍历在[offset,highest_bit]之间是否有空闲页槽索引*/
	...
	/*遍历在[lowest_bit, scan_base]之间是否有空闲页槽索引*/
	offset = si->lowest_bit;
...
/**
 * get_swap_page:在所有活动的交换区中获得一个空闲的页槽索引
 */
swp_entry_t get_swap_page(void)
{
	...
	for (type = swap_list.next; type >= 0 && wrapped < 2; type = next) {
		...
		/*同级的交换区,保存下一次进行调用的交换区索引，
		 * 这样就可以进行轮寻获得一个空闲的页槽索引*/
		swap_list.next = next;
...
/**
 * try_to_free_swap:page从交换区高速缓存中删除
...
/**
 * unuse_pte:给page映射页表项(页表项的地址有addr决定)
...
/**
 * try_to_unyse:将type所指的交换区的页框换入到RAM中
 */
static int try_to_unuse(unsigned int type)
{
	...
	while ((i = find_next_to_unuse(si, i)) != 0) {
		...
		if (swap_count(*swap_map) &&
			...
			/*page属于页交换高速缓存,并且该页为脏,则将其回写到磁盘上*/
...
/**
 * destroy_swap_extents:删除sis交换区的所有子区
 */
...
SYSCALL_DEFINE2(swapon, const char __user *, specialfile, int, swap_flags)
{
	...
	/*swap_info交换区插入到swap_list链表中
	 * 链表的顺序为,优先级高的节点位于优先级低的节点的前面
	 */
	prev = -1;
	for (i = swap_list.head; i >= 0; i = swap_info[i].next) {
		if (p->prio >= swap_info[i].prio) {
...
out:
	...
	if (did_down) {
		if (!error)
			inode->i_flags |= S_SWAPFILE; /*文件标记为交换区，文件当作交换区使用*/
		mutex_unlock(&inode->i_mutex);
	}
	return error;
}
