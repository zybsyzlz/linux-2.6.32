...
/*管理加入到zone LRU链表的page临时缓存，也就是说
 * 当page加入LRU链表时，并不是立刻加入，而是
 * 先放置到临时缓存中，当临时缓存满时，才进行
 * 正真的加入操作，这样可以提高系统的执行效率,
 * 减少了获取和释放自旋锁的开销,因为,只有当从页
 * 临时缓存刷新时,才会获得lru自旋锁
*/
...
/**
 * pagevec_move_tail:pvec中的缓存页加入到zone的lru类型的LRU链表的尾部
 */
...
/**
 * activate_page: page页放入zone的active LRU链表中
 */
...
/**
 * lru_cache_add_lru:page加入到页临时缓存中
 */
...
/**
 * drain_cpu_pagevecs:刷新Per-CPU的页临时缓存
 */
